...
def extract_traces(log: pd.DataFrame, traceid_key: str=constants.DEFAULT_TRACEID_KEY, activity_key: str=constants.DEFAULT_NAME_KEY) -> np.ndarray:
...
def extract_traces_activity_service_times(log: pd.DataFrame, num_bins: int=3, traceid_key: str=constants.DEFAULT_TRACEID_KEY, activity_key:str=constants.DEFAULT_NAME_KEY, start_time_key: str=constants.DEFAULT_START_TIMESTAMP_KEY, end_time_key: str=constants.DEFAULT_TIMESTAMP_KEY, seed: int | None=None) -> np.ndarray:
    ...
    out = log.groupby("case:concept:name").apply(lambda group_df: tuple(
                (evt[activity_key], (evt[end_time_key] - evt[start_time_key]).total_seconds())
    ...
    centroid, _ = kmeans_plusplus(durations, n_clusters=num_bins, n_local_trials=10, random_state=seed)
...
def weightedLevenshteinDistance(
    ...
    dist1 = weightedLevenshteinDistance(trace1[:-1],trace2[:-1],rename_cost,insertion_deletion_cost,cost_time_match_rename,cost_time_insert_delete)
...
def calc_timing_emd(distribution1: list[tuple[tuple[tuple[str, int], ...], float]], distribution2: list[tuple[tuple[tuple[str, int], ...], float]]) -> float:
    ...
    for i, (trace1, _) in enumerate(distribution1):
        for j, (trace2, _) in enumerate(distribution2):
            if (i,j) not in distances:
                distances[(i,j)] = postNormalizedWeightedLevenshteinDistance(trace1, trace2, rename_cost=lambda x,y: 1, insertion_deletion_cost=lambda x: 1, cost_time_match_rename=lambda x,y: abs(x-y), cost_time_insert_delete=lambda x: x)
            progress.update()
...
def compare_logs_emd(
    ...
    pop1 = Counter(extract_traces_activity_service_times(log_1, 3, traceid_key, activity_key, start_time_key, end_time_key, seed))
    pop2 = Counter(extract_traces_activity_service_times(log_2, 3, traceid_key, activity_key, start_time_key, end_time_key, seed))
...
def process_comparison_emd(
    ...
    emd = compare_logs_emd(log_1, log_2, traceid_key, activity_key, start_time_key, end_time_key, seed)
    ...
    for _ in range(5):
        ...
        emds.append(compare_logs_emd(
            *split_log_cases(log_1, 0.5, traceid_key),
            traceid_key, activity_key, start_time_key, end_time_key, seed
        ))
